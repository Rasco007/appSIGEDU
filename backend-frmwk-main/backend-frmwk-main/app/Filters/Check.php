...
class Check implements FilterInterface{
    ...
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        if($request->getCookie('accessToken') == null){
            return $crosFilter->sendResponse(['message' => 'Nonexistent Token'], 401);
        }
        ...
        if($vars->code >= 400){
            ...
            return $crosFilter->sendResponse(['message' => 'Invalid authorization'], $vars->code);
        }

        /** valida tiempo de vida de token */
        /** verificar el entorno que tmb deberia estar en el token */
        $valido = $this->checkStatus($vars, $firstUri);
        if($valido->code >= 400){
            // Eliminamos cookie HTTP ONLY
            CookieHelper::setCookie('accessToken', '', 0);
        ...
        $session = $this->checkActiveSession($request->header('cli')->getValue(), $token, $vars);
        if($session->code >= 400){
            // Eliminamos cookie HTTP ONLY
            CookieHelper::setCookie('accessToken', '', 0);

            return $crosFilter->sendResponse(['message' => 'Session expired.'], $session->code);
        }
    ...
    private function checkstatus($vars, $uriEntorno){
        /** todo valida tiempo de vida de token */
        $ret = new \stdClass();
        $model = new TokenModel();
        $now = time();
        $timeLimit = $model->getExpirationTime();
...
//        if($vars->exp <= $expiration){
    ...
    private function refreshtoken($token){
        ...
        if($vars->exp < $expiration){
            ...
            $registerLogin = $model->registerLoginUser($vars->user, $resp->token);
    ...
    private function checkActiveSession($clientKey, $accessToken, $vars){
        helper('tdi_helper');
        $ret = new \stdClass();
        $model = new TokenModel();

        $secret = $model->getSessionRecord($clientKey, decryptedAES($vars->user), $accessToken);
