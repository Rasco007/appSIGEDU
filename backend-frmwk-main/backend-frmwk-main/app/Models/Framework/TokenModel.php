...
class TokenModel extends Model{
    ...
    public function getSecret(){
        $query = $this->db->query("SELECT gt.d_dato1 as DATA FROM tablas_generales gt WHERE gt.n_tabla = 901 AND gt.c_dato = 'TOKEN_SECRET_KEY'");
        $arr = $query->getResultArray();
        return $arr[0]['DATA'];
    }

    public function getSessionRecord($clientKey, $userName, $accessToken){
        $builder = $this->db->table('clientes_servicio');
        $query = $builder->select('id_cliente')
            ->where('decryptedCode(c_clave)', $clientKey)
            ->where('m_habilitado', true)
            ->get();
        $clientId = $query->getRow()->clientId;
